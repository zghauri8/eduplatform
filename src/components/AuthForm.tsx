  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        // For demo purposes, use the response if available, otherwise create mock data
        const userData = response?.user || {
          id: `user-${Date.now()}`,
          name: formData.name,
          email: formData.email,
          role: formData.role as 'TVET' | 'ADOF',
        };
        ...
        const redirectPath = userData.role === 'ADOF' ? '/adof-dashboard' : '/dashboard';
        navigate(redirectPath);
        
        toast({
          title: 'Account created successfully!',
          description: `Welcome to ${userData.role} dashboard, ${userData.name}!`,
        });
        ...
        // Use the response if available, otherwise create mock data
        let userData;
        ...
        if (response?.user) {
          ...
          const isADOF = email.includes('adof') || email.includes('admin');
          userData = {
            id: `user-${Date.now()}`,
            name: formData.email.split('@')[0],
            email: formData.email,
            role: isADOF ? 'ADOF' as const : 'TVET' as const,
          };
        }
        ...
        // Redirect based on role
        navigate(userData.role === 'ADOF' ? '/adof-dashboard' : '/dashboard');
        
        toast({
          title: `Welcome back, ${userData.name}!`,
          description: `You're now signed in as ${userData.role}.`,
        });
      }
    } catch (error) {
      toast({
