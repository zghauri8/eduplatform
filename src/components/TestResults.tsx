...
import { Trophy, BarChart3, ArrowLeft, RotateCcw, BookOpen } from 'lucide-react';
...
export const TestResults: React.FC<TestResultsProps> = ({ 
  ...
  console.log('TestResults rendered with result:', result);
  const { data } = result;
  const { percentage = 0, total_score = 0, max_score = 0, analysis = {} } = data || {};
  
  // Debug log the analysis to see what weak areas we have
  console.log('Analysis data:', analysis);
  const weakAreas = Object.entries(analysis || {})
    .filter(([_, level]) => level === 'Weakness')
    .map(([trait]) => trait);
  console.log('Detected weak areas:', weakAreas);
